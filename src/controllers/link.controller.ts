...
export async function getProfileLinks(req: Request, res: Response) {
  try {
    const link = await linkService.getProfileLinks(req.params.id);
...
export async function getLinksByUsername(req: Request, res: Response) {
  try {
    const link = await linkService.getLinksByUsername(
...
export async function getLink(req: Request, res: Response) {
  try {
    ...
    if (!link) return res.status(404).json("Profile not found");
    return res.status(200).json(link);
  } catch (error) {
    console.log(error);
    return res.status(500).json(error);
